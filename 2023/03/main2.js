...
for (let i = 0; i < lines.length; i++){
	...
	for (let j in line){
		...
		if(isNumericSymbol(lines, j, i)){
			len ++;
			if(j == line.length-1) {
				managePartNumber(lines, startPos, i, len, parseInt(line.substring(startPos, startPos+len)))
			}	
			continue;
...
function managePartNumber(grid, x, y, length, number){
	let coords = isPartOfGear(grid, x, y, length);
	if(coords == false) return;
	coords.forEach(coord => {
		coord = JSON.stringify(coord);
		if(gears[coord] == undefined) gears[coord] = [];
...
function isPartOfGear(grid, x, y, length){

	let r = []

	for(let i of [-1, 1]){
		for (let j = -1; j < length + 1; j++){
			if(isGearSymbol(grid, x + j, y + i)) r.push([x+j, y+i]);
		}
	}

	if(isGearSymbol(grid, x - 1, y)) r.push([x-1, y]);
	if(isGearSymbol(grid, x + length, y)) r.push([x+length, y]);

	return r.lentgh == 0 ? false : r;
}
...
function isGearSymbol(grid, x, y) {
