...
// Djeco ne koristite sinatksu na koju ste slucajno naisli na mozilla docsu, for j in <string> 
// incrementa j od 0 do <string>.length, ali sam j je string. Tko se sjetio da bi to trebalo tako radit?
...
for (let i = 0; i < lines.length; i++){
	...
	let startPos = 0;
	for (let j in line){
		if(isNumericSymbol(lines, j, i)){
			len ++;
			if(parseInt(j) == line.length-1) {
				sum += isPart(lines, startPos, i, len) * parseInt(line.substring(startPos, startPos+len));
			}	
			continue;
...
function isPart(grid, x, y, length){

	for(let i of [-1, 1]){
		for (let j = -1; j < length + 1; j++){
			if(isEngineSymbol(grid, x + j, y + i)) return true;
		}
	}

	if(isEngineSymbol(grid, x - 1, y) || isEngineSymbol(grid, x + length, y)) return true;
...
function isNumericSymbol(grid, x, y){
	if("0123456789".includes(symbol(grid, x, y))) return true;
	return false;
}

function isEngineSymbol(grid, x, y) {
	let s = symbol(grid, x, y)
