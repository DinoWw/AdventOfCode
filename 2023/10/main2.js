...
const value = valueGen(grid);

// start point
let sp;

grid.forEach((l, y) => l.forEach((p, x) => {
	if(p == 'S'){
		sp = [x, y];
	}
}))
...
for(let i = 0; i < 4; i ++){
	let pos = [sp[0]+dirs[i][0], sp[1]+dirs[i][1]];
	let id = pipes[value(pos)].indexOf((i+2)%4)
	if(id != -1){
		path.push([pos, pipes[value(pos)][(id + 1) % 2]]);
		path[0][1] = i;
		break;
	}
}
...
do{
	...
	let pos = [last[0][0]+dirs[last[1]][0], last[0][1]+dirs[last[1]][1]];
	...
	let id = pipes[pipe].indexOf((last[1]+2)%4)
	if(id == -1) console.log("ERROR")
	path.push([pos, pipes[pipe][(id + 1) % 2]]);
...
const cycle = path.map(x => x[0]);
/*

let rowed = []
for(let i = 0; i < grid.length; i++){
	rowed.push(cycle.filter(x => x[1] == i));
}



rowed.forEach(l => l.sort((a, b) => a[0]-b[0]))
...
let area = -cycle.length/2+1;

for(let i = 1; i < cycle.length; i ++){
	let pos = cycle[i];
	area += -pos[1]*(pos[0] - cycle[i-1][0]);
}
...
function valueGen(field){
	return pos => field[pos[1]][pos[0]];
}
